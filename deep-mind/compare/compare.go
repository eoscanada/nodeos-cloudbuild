...
func init() {
	logging.Register("battlefield", &zlog)
...
func main() {
	ensure(len(os.Args) == 2, "Single argument must be <target> folder to compare")
	target := os.Args[1]
	...
	for {
		...
		if err != nil {
			if lastBlockRead == nil {
				...
				noError(err, "Unable to read block from file %q, last block read was %s", filePath, lastBlockRead.AsRef())
			}
		}
	}
